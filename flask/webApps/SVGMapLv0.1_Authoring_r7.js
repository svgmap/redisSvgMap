...
//		nextsibling.....? なんか無造作すぎる気もする・・・
	...
	console.log("poiElem:",poiElem,"    uiMapping.modifyTargetElement:",uiMapping.modifyTargetElement);
	var param = poiParams;
...
//	document.removeEventListener("click", setPoiRegPosition, false);
	if ( !directPutPoiParams ){
		...
		if ( uiMapping.toolsCbFunc ){
			uiMapping.toolsCbFunc(true, uiMapping.toolsCbFuncParam);
		}
	}
...
//		console.log("addPoint:",point,geoPoints);
	...
	function setPoints(points , objIsPolygon){
		//console.log("setPoints:",setPoints.caller);
		if ( points[0].lat ){
			geoPoints = points;
		} else {	
			geoPoints=[];
...
return { // svgMapAuthoringTool. で公開する関数のリスト
	editPoint: editPoint,
