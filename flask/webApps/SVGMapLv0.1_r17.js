...
//		console.log("dispatch POI event");
	...
	**/
		...
		function endPan() {
			...
			if (panning) {
				panning = false;
				if (shiftZoomingBox && shiftZoomingBox.style.display == "") {
					...
					if (evt.type.indexOf("touch") >= 0) {
						...
						if (zoomingTransitionFactor != -1) {
							zoomingTransitionFactor =
								getTouchDistance(evt) / initialTouchDisance;
						}
					} else {
			...
			if (panning) {
				if (!shiftZoomingBox) {
					...
					document
						...
						.insertAdjacentElement("afterend", shiftZoomingBox);
				}
...
//		console.log ( x0 , y0 );
	...
	**/
		...
		function smoothZoom(zoomFactor, startDate, doFinish, startZoom) {
			...
			if (!doFinish) {
				...
				if (elapsedTime < smoothZoomTransitionTime) {
					...
					zoomingTransitionFactor =
						...
						(1 / zoomFactor - 1 / startZoom) *
							(elapsedTime / smoothZoomTransitionTime);
					...
					if (typeof requestAnimationFrame == "function") {
						requestAnimationFrame(function () {
							smoothZoom(zoomFactor, startDate, false, startZoom);
						});
					} else {
						setTimeout(
							smoothZoom,
					...
					if (typeof requestAnimationFrame == "function") {
						requestAnimationFrame(function () {
							smoothZoom(zoomFactor, startDate, true, startZoom);
						});
					} else {
						setTimeout(
							smoothZoom,
				...
				if (additionalZoom != 0) {
					...
					if (typeof requestAnimationFrame == "function") {
						requestAnimationFrame(function () {
							smoothZoom(azf, new Date(), false, zoomFactor);
						});
					} else {
						setTimeout(
							smoothZoom,
	...
	//			++ removedImgs;
		...
		function setLayerDivProps(id, parentElem, parentSvgDocId) {
			...
			if (parentSvgDocId) {
				if (parentSvgDocId == "root") {
					...
					svgImagesProps[id].rootLayer =
						svgImagesProps[parentSvgDocId].rootLayer;
					parentElem.setAttribute(
						"class",
						"rootLayer:" + svgImagesProps[parentSvgDocId].rootLayer
					);
				}
			}
		}
		...
		function loadSVG(path, id, parentElem, parentSvgDocId) {
			...
			if (!svgImages[id]) {
				...
				var httpObj = createXMLHttpRequest(
					function () {
						handleResult(id, path, parentElem, this, parentSvgDocId);
					},
					function () {
						handleErrorResult(id, path, this, true);
					}
				);
				...
				if (httpObj) {
					...
					if (
						svgImagesProps[id].rootLayer &&
						svgImagesProps[svgImagesProps[id].rootLayer].noCache
					) {
					...
					const tempComQuery =
						svgImagesProps[id].commonQuery ||
						svgImagesProps[svgImagesProps[id].rootLayer]?.commonQuery; // ?. : オプショナルチェーンという書き方でアクセス元がNullでも途中でエラーとならない書き方
					...
					if (typeof contentProxyParams.getUrlViaProxy == "function") {
						// original 2014.2.25 by konno (たぶん)サイドエフェクトが小さいここに移動 s.takagi 2016.8.10
					...
					if (uaProp.MS && httpObj.ontimeout) {
		...
		function getControllerSrc(resTxt, svgImageProps) {
			// 2017.2.21
		...
		function handleResult(docId, docPath, parentElem, httpRes, parentSvgDocId) {
			...
			**/
				...
				if (
					httpRes.responseXML != null &&
					httpRes.responseXML.documentElement &&
					!uaProp.MS &&
					verIE >= 100 &&
					isSvg(httpRes.responseXML) &&
					httpRes.responseText.indexOf("<script>") < 0
				) {
					...
					if (httpRes.responseText.indexOf("http://www.w3.org/2000/svg") >= 0) {
						...
						if (resTxt.match(/<script>([\s\S]*)<\/script>/)) {
							// 2015.9.11 動的コンテンツでXML特殊文字がエスケープされていないスクリプトの処理
						...
						svgImages[docId] = new DOMParser().parseFromString(
							resTxt,
							"application/xml"
						);
					} else {
				...
				if (docId == "root") {
					...
					rootViewBox = getrootViewBoxFromRootSVG(
						viewBox,
						mapCanvasSize,
						ignoreMapAspect
					);
				...
				if (svgImagesProps[docId].script) {
					var zoom = getZoom(
						getRootSvg2Canvas(rootViewBox, mapCanvasSize),
						docId
					);
					var child2root = getConversionMatrixViaGCS(
						svgImagesProps[docId].CRS,
						rootCrs
					);
					...
					svgImagesProps[docId].script.location = getSvgLocation(
						svgImagesProps[docId].Path
					);
					...
					if (typeof svgMapGIStool == "object") {
						svgImagesProps[docId].script.drawGeoJson =
		...
		function dynamicLoad(docId, parentElem) {
			...
			if (docId == "root") {
				...
				if (!setRootLayersPropsPostprocessed) {
			...
			if (docId == "root") {
				...
				if (ticker && !pathHitTest.enable && !GISgeometriesCaptureFlag) {
		...
		function handleScript(docId, zoom, child2root) {
			...
			svgImagesProps[docId].script.actualViewBox = getTransformedBox(
				rootViewBox,
				getInverseMatrix(child2root)
			); // *ViewBoxは間違い・viewportが正しい・・互換のために残す・・・
			svgImagesProps[docId].script.geoViewBox = geoViewBox;
			svgImagesProps[docId].script.viewport =
				svgImagesProps[docId].script.actualViewBox; // debug 2014.08.06
			...
			if (vc == "zoom" || svgImagesProps[docId].script.initialLoad) {
				// zooming
			...
			if (
				svgImagesProps[docId].refresh.timeout > 0 &&
				svgImagesProps[docId].refresh.loadScript == true
			) {
		...
		function handlePreRenderControllerScript(
			docId,
			zoom,
			child2root,
			isSuperController
			...
			var svgDocStatus = {
				...
				actualViewBox: getTransformedBox(
					rootViewBox,
					getInverseMatrix(child2root)
				),
...
**/
		...
		function viewBoxChanged(docId) {
			...
			var ans;
			if (
				!prevRootViewBox[docId] ||
				prevRootViewBox[docId].width != rootViewBox.width ||
				prevRootViewBox[docId].height != rootViewBox.height
			) {
				ans = "zoom";
			} else if (
				prevRootViewBox[docId].x != rootViewBox.x ||
				prevRootViewBox[docId].y != rootViewBox.y
			) {
				ans = "scroll";
			} else {
				ans = false;
			...
			prevRootViewBox[docId] = {
				x: rootViewBox.x,
				y: rootViewBox.y,
				width: rootViewBox.width,
				height: rootViewBox.height,
			};
			return ans;
		}
		...
		function parseSVG(
			...
			for (var i = 0; i < svgNodes.length; i++) {
				...
				switch (svgNode.nodeName) {
					...
					case "iframe":
						...
						break;
					case "image":
						if (getNonScalingOffset(svgNode).nonScaling) {
				...
				if (!pathHitTest.enable && GISgeometriesCaptureFlag) {
					GISgeometry = initGISgeometry(
						childCategory,
						childSubCategory,
						svgNode
					); // 2016.12.1 for GIS  : geoJsonのgeom蓄積
				}
				...
				if (
					(!pathHitTest.enable &&
						(childCategory == POI ||
							...
							childCategory == EMBEDSVG ||
							childCategory == TEXT)) ||
					(pathHitTest.enable && childCategory == EMBEDSVG)
				) {
					...
					if (
						imageRect.width == 0 &&
						imageRect.height == 0 &&
						(childCategory == EMBEDSVG || childCategory == BITIMAGE)
					) {
						console.warn(
					...
					if (ip.commonQuery) {
						if (docId != "root") {
							...
							svgImagesProps[svgImagesProps[docId].rootLayer].commonQuery =
								ip.commonQuery;
						}
					}
			...
			**/
					...
					if (
						!eraseAll &&
						isIntersect(imageRect, rootViewBox) &&
						inZoomRange(ip, zoom, imageRect.c2rScale) &&
						isVisible(ip)
					) {
						// ロードすべきイメージの場合
						...
						if (ip.transform) {
							...
							if (childCategory == POI && childSubCategory == USEDPOI) {
								// ICON表示
						...
						if (!imgElem) {
							...
							var img;
							if (childCategory == POI || childCategory == BITIMAGE) {
								// image,use要素の場合
								var imageURL = getImageURL(ip.href, docDir);
								var isNoCache =
									childCategory == BITIMAGE &&
									svgImagesProps[docId].rootLayer &&
									svgImagesProps[svgImagesProps[docId].rootLayer].noCache;
								img = getImgElement(
									xd.p0,
									yd.p0,
									xd.span,
								...
								if (ip.opacity) {
									...
									if (!uaProp.MS) {
										...
										img.style.filter =
											"alpha(opacity=" + ip.opacity * 100 + ")"; // IEではこれだけでは効かない
								...
								if (
									svgImagesProps[docId].isClickable ||
							...
							if (
								childCategory == POI &&
								GISgeometriesCaptureOptions.SkipVectorRendering
							) {
								// POIもベクタとして描画しない
							} else {
							...
							if (isIE) {
								...
								if (
									verIE < 9 &&
									(childCategory == POI || childCategory == BITIMAGE)
								) {
							...
							if (childCategory == POI || childCategory == BITIMAGE) {
								...
								if (
									svgImagesProps[imageId] &&
									svgImagesProps[imageId].Path &&
									svgImagesProps[imageId].Path != childSVGPath
								) {
					...
					**/
					...
					if (
						...
						if (bbox) {
							...
							if (
								(pathHitTest.enable || pathHitTest.centralGetter) &&
								bbox.hitted
							) {
		...
		function getSvgReq(href) {
			// ハッシュなどの扱いをきちんとした 2014.6.27
		...
		function parseSVGwhenLoadCompleted(svgImages, imageId, imgElem, ct) {
			...
			if (svgImagesProps[imageId] && svgImagesProps[imageId].loadError) {
				// 2021/2/17 ERR404強化
				return;
			}
			if (svgImages[imageId] && svgImagesProps[imageId]) {
				...
				var symbols = getSymbols(svgImages[imageId]);
				parseSVG(
					svgImages[imageId].documentElement,
					imageId,
					imgElem,
					false,
					symbols,
		...
		function removeEmptyTiles(parentNode) {
							...
							if (symbolNodes[k].hasChildNodes) {
								for (var l = 0; l < symbolNodes[k].childNodes.length; l++) {
									if (symbolNodes[k].childNodes[l].nodeName == "path") {
										var symb = getPathSymbolMakerProps(
		...
		function getrootViewBoxFromRootSVG(
			viewBox,
			mapCanvasSize_,
			ignoreMapAspect
		) {
		...
		function getImgElement(
			...
			if (typeof contentProxyParams.getUrlViaImageProxy == "function") {
				// 2020.1.30 image用のproxyが使えるようにする
				...
				typeof contentProxyParams.getNonlinearTransformationProxyUrl ==
					"function" &&
				needsNonLinearImageTransformation(
					svgImagesProps[svgimageInfo.docId].CRS,
		...
		function setLoadingImagePostProcessing(
			img,
			href,
			id,
			forceSrcIE,
	...
	**/
		...
		function setImgElement(
			...
			if (!txtFlg && img.src && href && isHrefChanged(imgSrc, href)) {
		...
		**/
				if (globalView && globalView.nodeName == "view") {
					...
					var gcsElem = svgDoc.getElementsByTagName(
					...
					if (gcsElem) {
						...
						**/
								...
								// 2020/3/24 add mercator support
								...
								var cntlWin =
									svgImagesProps[svgImagesProps[docId].rootLayer]
										.controllerWindow;
								if (cntlWin) {
							...
							} else if (svgImagesProps[docId].script) {
								// こちらは動作する実装
			...
			if (svgImageProps.controller) {
				if (cntPath) {
					svgImageProps.controller =
						"hash:" + cntPath + ":" + svgImageProps.controller;
				...
				if (cntPath) {
					...
					if (svgImageProps["parentDocId"] == "root") {
						cntPath = getLayer(svgImageProps["rootLayer"]).getAttribute(
							"data-controller"
						);
						if (
							!(cntPath === null || cntPath === undefined || cntPath === "")
						) {
							svgImageProps.controller = getImageURL(
								cntPath,
								getDocDir(docPath)
							);
						}
...
//			scale = (1/ifCrs.scale) * toCrs.scale; // インバースのインバースになってる・これはバグだと思う 2020/6/9
		...
		**/
		...
		// 2020/10/20 整理
		function transformRect_duplicated(rect, c2r) {
			// 廃止
		...
		function getZoom(s2c, docId) {
			...
			if (layerDevicePixelRatio[layerId] != undefined) {
				ans =
					(Math.abs(s2c.a) + Math.abs(s2c.d)) /
					(2.0 * layerDevicePixelRatio[layerId]);
			} else {
				ans =
					(Math.abs(s2c.a) + Math.abs(s2c.d)) / (2.0 * commonDevicePixelRatio);
			}
		...
		function getImageProps(
			...
			if (
				category == EMBEDSVG ||
				category == BITIMAGE ||
				subCategory == DIRECTPOI
			) {
				if (category == EMBEDSVG && subCategory == SVG2EMBED) {
					// svg2のsvgインポート
					//			console.log(imgE);
				...
				**/
					...
					if (GISgeometry) {
						if (category == BITIMAGE && !nonScaling) {
							// 2018.2.26
						...
						} else if (subCategory == DIRECTPOI) {
							// 2018.3.2 上の話を改修した部分
							GISgeometry.svgXY = [x, y];
						}
					}

					if (subCategory == DIRECTPOI) {
						// 2018.3.2
...
//				console.log("use Microsoft.XMLHTTP");
		...
		**/
		...
		function getAjaxFilter() {
			if (navigator.appVersion.indexOf("KHTML") > -1) {
				return function (t) {
					var esc = escape(t);
					return esc.indexOf("%u") < 0 && esc.indexOf("%") > -1
		...
		function setLayerListSize() {
			...
			if (!llElemSize || llElemSize.indexOf("px") < 0) {
				llElem.style.width =
					mapCanvasSize.width * 0.5 - spButtonSize - 5 + "px";
			...
			if (
				spButtonSize + 5 + Number(llElem.dataset.originalSize) >
				mapCanvasSize.width
			) {
	...
	**/
		...
		function configIE() {
			...
			if (!("console" in window)) {
				// console.log issue when no dev window 2013/8/19
				window.console = {};
		...
		function screen2Geo(screenX, screenY) {
		...
		function geo2Screen(lat, lng) {
			...
			return {
				x:
					((rootXY.x - rootViewBox.x) * mapCanvasSize.width) /
					rootViewBox.width,
				y:
					((rootXY.y - rootViewBox.y) * mapCanvasSize.height) /
					rootViewBox.height,
			};
		}
		...
		function setGeoViewPort(lat, lng, latSpan, lngSpan, norefresh) {
			...
			rootViewBox = getrootViewBoxFromGeoArea(
				lat,
				lng,
				latSpan,
				lngSpan,
		...
		function getrootViewBoxFromGeoArea(
			lat,
			lng,
			latSpan,
			lngSpan,
			...
			if (gpsb) {
				if (navigator.geolocation) {
					...
					if (
						...
						script.innerHTML =
							"function gps(){navigator.geolocation.getCurrentPosition( gpsSuccess );}function gpsSuccess(position){window.parent.svgMap.gpsCallback(position)}";
		...
		**/
		...
		function getLayer(layerID_Numb_Title) {
			...
			if (isNaN(layerID_Numb_Title)) {
				...
				if (isSVG2) {
					layer =
						svgImages["root"].getElementsByTagName("iframe")[
							layerID_Numb_Title
						];
				} else {
					layer =
						svgImages["root"].getElementsByTagName("animation")[
							layerID_Numb_Title
						];
				}
			}
		...
		function checkLayerSwitch(selectedLayer) {
			...
			if (selectedLayer.getAttribute("class")) {
				...
				selectedLayerClass = removeLayerCatName(
					selectedLayerClass,
					"batch",
					"editable",
					"clickable",
		...
		function getRootLayersProps() {
			...
			for (var i = 0; i < layers.length; i++) {
				...
				if (
					svgImagesProps[layersProps[i].id] &&
					svgImages[layersProps[i].id] &&
					!loadingImgs[layersProps[i].id]
				) {
				...
				var layerGroupName = "";
				if (layers[i].getAttribute("class")) {
					var layerGroupNames = removeLayerCatName(
	...
	**/
		...
		function checkTicker(px, py) {
			...
			if (px && py) {
				...
				hittedPoiObjects = getPoiObjectsAtPoint(
					mapCanvasSize.width / 2,
					mapCanvasSize.height / 2
				); // ラスタPOIでヒットしたモノ
				hittedLayerHitTests = getLayerHitTestAtPoint(
					mapCanvasSize.width / 2,
					mapCanvasSize.height / 2,
					true
			...
			if (
				...
				px &&
				py &&
				checkAndKickEditor(hittedObjects, px, py)
			) {
			...
			if (
				...
				if (px && py && tickerTableMetadata.length == 1) {
	...
	**/
			...
			if (needsNonLinearImageTransformation(crs, imageElem) == false) {
				// 2021/08/10
				return;
			}
	...
	**/
		...
		function timeoutLoadingImg(obj) {
		...
		function requestRemoveTransition(imgElem, parentElem2) {
		...
		function buildPixelatedImages4Edge() {
		...
		function checkLoadCompleted(forceDel) {
			...
			if (hl == 0 || forceDel) {
				...
				if (!loadCompleted && !pathHitTest.enable) {
					...
					if (viewBoxChanged()) {
						...
						for (var key in svgImagesProps) {
							// 2017.3.9 scriptを持つsvg文書にもzpmイベントを送る 同じ仕組みで他のイベント的なものにも本物のイベントを送れる
		...
		function startRefreshTimeout() {
			for (var layerId in svgImagesProps) {
				if (
					svgImagesProps[layerId].refresh &&
					svgImagesProps[layerId].refresh.timeout > 0
				) {
					if (svgImagesProps[layerId].refresh.start == false) {
						...
						setTimeout(
							refreshLayer,
							svgImagesProps[layerId].refresh.timeout * 1000,
							layerId
						);
					} else {
		...
		function getHashLength(arr) {
			// Arrayの個数を調べる( hashのため )
			var cnt = 0;
			for (var key in arr) {
				cnt++;
			}
			if (verIE < 9) {
				// polyfillでindexOfを追加してるため・・
				--cnt;
			}
			return cnt;
		}
...
**/
		...
		function getImagePath(inDocPath, docId) {
			// ルート文書に対する子文書の相対位置を加味したパスを得る getImageURLと類似していないか？（この関数は現在使われていません・・・）
			var imageURL;
		...
		function getNonScalingOffset(svgPoiNode) {
				...
				function (e) {
					switch (e.target.id) {
						case "txtAreaCSV": // 値設定決定用
							body.innerHTML = escape(
								svgPoi2csv(poi.ownerDocument.documentElement)
							);
							break;
		...
		function getElementByAttr_obsoluted(XMLNode, searchId, atName) {
			// Firefox用・・（IE11でも同じことがおきる場合がある 2014.6.20)
			...
			for (var k = 0; k < XMLNodes.length; k++) {
				...
				if (
					XMLNodes[k].getAttribute &&
					XMLNodes[k].getAttribute(atName) == searchId
				) {
					return XMLNodes[k];
				}
		...
		function poiSelectProcess(obj) {
			...
			if (
				typeof svgMapAuthoringTool == "object" &&
				el &&
				el.getAttribute("iid") ==
					svgImagesProps[target.parentNode.getAttribute("id")].rootLayer
			) {
		...
		function processShowUse(svgTargetObj) {
			var svgTarget = svgTargetObj.element;
			if (getHyperLink(svgTarget) && !svgTarget.getAttribute("content")) {
			...
			} else if (getHyperLink(svgTarget) && svgTarget.getAttribute("content")) {
				// アンカーもあってメタデータもある場合
		...
		function getPropsOfPoi(poiId) {
			...
			if (
				...
				var layerId = screenPOIimg.parentNode
					.getAttribute("class")
					.substring(10);
		...
		function getLayerHash(layerName) {
		...
		function svgPoi2csv(svgDocElement) {
			...
			for (var i = 0; i < pois.length; i++) {
				...
				ans +=
					numberFormat(geoPos.lat) +
					"," +
					numberFormat(geoPos.lng) +
					"," +
					poiProp.href +
					"," +
					poiProp.title +
		...
		function setSVGpathPoints(
			...
			**/
			...
			if (
				...
				if (
					context.isPointInPath(pathHitTest.x, pathHitTest.y) ||
					context.isPointInStroke(pathHitTest.x, pathHitTest.y)
				) {
			...
			if (
				...
				if (pathHitTest.enable || pathHitTest.centralGetter) {
					// ヒットテスト要求時の線検索
					if (context.isPointInStroke(pathHitTest.x, pathHitTest.y)) {
		...
		function isCollided(POIelemId) {
			...
			try {
				...
				for (var i in visiblePOIs) {
					if (i != POIelemId) {
						if (
							targetPOI.x + targetPOI.width < visiblePOIs[i].x ||
		...
		function setCentralVectorObjectsGetter() {
			// checkTicker()(画面中心のデフォルトヒットテスト)での二重パースを防止するための関数 2018.1.18
			if (!pathHitTest.enable) {
				// getVectorObjectsAtPoint(x,y)が要求されていた時はこの機能を発動させてはまずい
				...
				if (
					typeof svgMapAuthoringTool == "object" &&
					svgMapAuthoringTool.isEditingGraphicsElement()
				) {
					// オブジェクトを編集中には、ジェネラルなヒットテストは実施しない
		...
		function getVectorMetadata(element, parent, bbox) {
			console.log(
				"called getVectorMetadata: ",
				element,
				parent,
				bbox,
				"  caller:",
				getVectorMetadata.caller
			);
			...
			var layerName = getLayerName(
				getLayer(
					svgImagesProps[element.ownerDocument.firstChild.getAttribute("about")]
						.rootLayer
				)
			);
			...
			if (typeof svgMapAuthoringTool == "object" && editingLayer) {
				for (var i = 0; i < hittedObjects.elements.length; i++) {
					if (
						...
						getDocumentId(hittedObjects.elements[i])
					) {
						editingTarget = i;
						break;
					}
		...
		function vectorDataWrapperForShowPoiProperty(
			targetElement,
			targetBbox,
			usedParent
		) {
			...
			if (usedParent && usedParent.getAttribute("content")) {
				targetElement.setAttribute(
					"content",
					usedParent.getAttribute("content")
				);
			}
	...
	**/
		...
		function defaultShowPoiProperty(target) {
			...
			if (target.getAttribute("lat")) {
				message +=
					...
					getFormattedRange(target.getAttribute("lat")) +
					"</td></tr>";
				message +=
					"<tr><td>longitude</td> <td>" +
					getFormattedRange(target.getAttribute("lng")) +
					"</td></tr>";
			}
		...
		function editDOM(interval, clear) {
		...
		function setCookie(key, value, expire) {
			...
			var rootPath = new URL(svgImagesProps["root"].Path, location.href)
				.pathname;
			window.localStorage[localStorageSvgMapSuffix + rootPath + "#" + key] =
				value;
		}
				...
				if (cook.customLayers && window.svgMapCustomLayersManager) {
					try {
						...
						parseSVG(
							...
							null,
							true
						); // 2021/3/8 iidを設定する(上と同じ)
						lp = getRootLayersProps();
					} catch (e) {
						console.error(
							"svgMapCustomLayersManager.applyCustomLayers step error:",
							e
						);
					}
				}
				if (cook.customGeoViewboxes) {
		...
		**/
		...
		var GISgeometriesCaptureOptions = {
			// for GIS 2021.9.16
		...
		var contentProxyParams = {
			// プロキシ経由でコンテンツを取得するための設定オブジェクト
		...
		function setProxyURLFactory(
			...
			if (
				typeof imageURLviaProxyFunctionForNonlinearTransformation == "function"
			) {
				contentProxyParams.getNonlinearTransformationProxyUrl =
					imageURLviaProxyFunctionForNonlinearTransformation;
			} else if (imageURLviaProxyFunctionForNonlinearTransformation === null) {
				// undefinedのときは何もしないようにした方が良いかもということで 2021/1/27
		...
		function getCORSURL(originalURL, alsoCrossoriginParam) {
			if (alsoCrossoriginParam) {
				// SVGMap.jsとcrossorigin設定も同期させる場合にはその情報も得る（かわりobjectになる）
				if (
					typeof contentProxyParams.getNonlinearTransformationProxyUrl ==
					"function"
				) {
					return {
						url: contentProxyParams.getNonlinearTransformationProxyUrl(
							originalURL
						),
				...
				if (
					...
					return contentProxyParams.getNonlinearTransformationProxyUrl(
						originalURL
					);
		...
		function getLinearTransformMatrix(
			x1i,
			y1i,
			x2i,
			y2i,
			...
			// output:
			// matrix{.a,.b,.c,.d,.e,.f}
			var xs = getTernarySimultaneousEquationsSolution(
				x1i,
				y1i,
				1,
				x2i,
			...
			var ys = getTernarySimultaneousEquationsSolution(
				x1i,
				y1i,
				1,
				x2i,
		...
		function getTernarySimultaneousEquationsSolution(
			a11,
			a12,
			a13,
			a21,
		...
		return {
			...
			captureGISgeometriesOption: function (
				...
				if (TreatRectAsPolygonFlg === true || TreatRectAsPolygonFlg === false) {
					GISgeometriesCaptureOptions.TreatRectAsPolygonFlag =
						TreatRectAsPolygonFlg; // rect要素をPoint扱いにするかPolygon扱いにするか
				}
				if (
					SkipVectorRenderingFlg === true ||
					SkipVectorRenderingFlg === false
				) {
					GISgeometriesCaptureOptions.SkipVectorRendering =
						SkipVectorRenderingFlg;
				}
			...
			ignoreMapAspect: function () {
				ignoreMapAspect = true;
			},
			initLoad: initLoad,
			...
			setMapCanvas: function (mc) {
				mapCanvas = mc;
			},
			setMapCanvasCSS: setMapCanvasCSS,
			setMapCanvasSize: function (mcs) {
				mapCanvasSize = mcs;
			},
			setPreRenderController: function (layerId, pcf) {
				// SVGMapLv0.1_PWAで使用
			...
			setResume: function (stat) {
				...
				resume = stat;
				if (!resume || Object.keys(svgImages).length > 0) {
			...
			setSummarizeCanvas: function (val) {
				summarizeCanvas = val;
			},
