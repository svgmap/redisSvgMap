...
#  Rev16.1 2019/09/13 スキーマでデフォルトアイコンを設定できる
#
# How to use redis...
...
class Csv2redisClass():
  def __init__(self):
    ...
    self.poiColors = ["blue", "cyan", "green", "yellow", "orange", "pink", "red", "purple", "red_cross"]
  ...
  def saveSvgMapTileN(self,
    ...
    defaultColrCode = self.schemaObj.get("defaultIcon")
    if (defaultColrCode > len(self.poiColors)):
      defaultColrCode = 0
    ...
    if dtype == b"string":  # そのタイルはオーバーフローしている実データがないlowRes pickleタイル
      if lats < 360:  # レベル0のタイル(レイヤールートコンテナ)じゃない場合はそのレベルの低解像度タイルを入れる
        pixW = 100 * lngs / self.lowresMapSize
        pixH = 100 * lats / self.lowresMapSize
        outStrL.append("<g fill='" + self.poiColors[defaultColrCode] + "' visibleMaxZoom='{:.3f}'>\n".format((Csv2redisClass.topVisibleMinZoom * pow(2, thisZoom - 1))))
      ...
      outStrL.append(
          "<defs>\n <g id='p0'>\n  <image height='27' preserveAspectRatio='none' width='19' x='-8' xlink:href='pin_" +
          self.poiColors[defaultColrCode] + ".png' y='-25'/>\n </g>\n</defs>\n")
