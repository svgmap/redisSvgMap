...
def quadPart(ans, lat0, lng0, lats, lngs, latCol, lngCol):
  ...
  for i, childTile in enumerate(childTiles):
    if (childTile):
      if "total" in childTile:  # childもloweResMap
        ...
        updateLowResMapData(lowResMap, childTile, lat0, lng0, lats, lngs, latCol, lngCol)
...
def updateLowResMapData(parentLowResMap, childTile, lat0, lng0, lats, lngs, latCol, lngCol):
...
def burstCombine(dataSizes):
  ...
  combinedKeys = {}
  for key, count in dataSizes.items():
    if (len(key) > 1 and count < listLimit):
      pKey = key[0:-1]
...
def saveSvgMapTile(geoHash, dtype=None):  # pythonのXML遅くて足かせになったので、この関数は使わなくしました 2019/1/16
  ...
  csvSchemaType = schemaObj.get("type")
  svg.setAttribute("property", getCsvStrExclLatLng(csvSchema, latCol, lngCol))
  ...
  if dtype == b"string":  # そのタイルはオーバーフローしている実データがないlowRes pickleタイル
    ...
    for poidata in src:
      ...
      use.setAttribute("x", "0")
      use.setAttribute("y", "0")
      use.setAttribute("content", getCsvStrExclLatLng(poi, latCol, lngCol))
      svg.appendChild(use)
...
def getCsvStrExclLatLng(strList, latCol, lngCol):  # 配列から緯度経度カラムを除いたカンマ区切り文字列を作ります
  ans = []
  ...
  if dtype == b"string":  # そのタイルはオーバーフローしている実データがないlowRes pickleタイル
    if lats < 360:  # レベル0のタイル(レイヤールートコンテナ)じゃない場合はそのレベルの低解像度タイルを入れる
      ...
      if (lowResImage):  # 作ったpngを参照するimageタグを作る
        ...
        image.__setattr__("xlink:href", svgFileNameHd + geoHash + ".png")
    ...
    if lats < 360:
      g.fill = 'blue'
      g.visibleMinZoom = topVisibleMinZoom * pow(2, thisZoom - 1)
    ...
    for i, exs in enumerate(ceFlg):  # link to child tiles
      ...
      if (exs):
        ani = Tag("animation")
        ani.__setattr__("xlink:href", svgFileNameHd + childGeoHash + ".svg")
        lat_shift = 0
        lng_shift = 0
        if cN == "B":
          lng_shift = lngs / 2
